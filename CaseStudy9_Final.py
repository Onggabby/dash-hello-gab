...
def update_output(n_clicks, uname, passw):
    ...
    if uname =='' or uname == None or passw =='' or passw == None:
        output = html.Div(children='',style={'padding-left':'550px','padding-top':'10px'})
        pathname = "/"
        return [output,pathname]
    if uname not in li:
        output = html.Div(children='Incorrect Username',style={'padding-left':'550px','padding-top':'40px','font-size':'16px'})
        pathname = "/"
        return [output,pathname]
    if li[uname]==passw:
        output = html.Div(dcc.Link('Access Granted!', href='/next_page',style={'color':'#183d22','font-family': 'serif', 'font-weight': 'bold', "text-decoration": "none",'font-size':'20px'}),style={'padding-left':'605px','padding-top':'40px'})
        pathname = "/next_page"
        return [output, pathname]
